...
public class MotionEngine {
    ...
    private void pushLinearMove(float[] start_point, float[] end_point, float target_feedrate, float actual_feedrate)
    {
        float x_dist = (end_point[0] - start_point[0]);
        float y_dist = (end_point[1] - start_point[1]);
        float z_dist = (end_point[2] - start_point[2]);
        //System.out.println("x_dist: " + x_dist);
        //System.out.println("y_dist: " + y_dist);
        long[] feedrates = getIndividualAxisFeedrates(actual_feedrate, x_dist, y_dist, z_dist, distanceBetween3DPoints(start_point, end_point));
        StepGenStruct gen = new StepGenStruct();
        gen.x_step_count = (long)(Math.abs(x_dist) * (float)step_scale[0]);
        gen.x_total_step_count = gen.x_step_count;
        gen.x_dir = true;
        ...
        gen.y_delay = feedrates[1];
        gen.z_delay = feedrates[2];
        gen.target_velocity = target_feedrate;
        gen.current_velocity = actual_feedrate;
        gen.vector_angle = getAngle(start_point, end_point);
        gen.start_point = start_point;
        gen.end_point = end_point;
        move_buffer.add(gen);
    ...
    private void planMotionBuffer()
    {
        ...
        for (int x = 0; x < move_buffer_without_planning.size(); x++)
        {
            if (move_buffer_without_planning.get(x).z_step_count == 0) //We are an XY move
            {
                ...
                float last_moves_current_velocity = 0;
                if (move_buffer_without_planning.get(x-1) != null)
                {
                    last_moves_current_velocity = move_buffer_without_planning.get(x-1).current_velocity;
                }
                float time_needed_to_accelerate_to_target = TimeNeededToAccelerateToTargetVelocity(move_buffer_without_planning.get(x).target_velocity - last_moves_current_velocity, axis_acceleration[0]);
                ...
                if (time_needed_to_accelerate_to_target > 0) //We need to accelerate
                {
                    if (move_buffer_without_planning.size() > x+1) //There is another move
                    {
                        ...
                        float acceleration_iteration_steps = 10;
                        for (float i = time_needed_to_accelerate_to_target / acceleration_iteration_steps; i < time_needed_to_accelerate_to_target; i += time_needed_to_accelerate_to_target / acceleration_iteration_steps)
                        {
                            ...
                            move_buffer_without_planning.get(x).current_velocity = VelocityAtCurrentIteration(total_distance, i);
                            System.out.println("Current Velocity: " + move_buffer_without_planning.get(x).current_velocity + " Target Velocity: " + move_buffer_without_planning.get(x).target_velocity);
                            float[] end_point = getPolarLineEndpoint(last_endpoint, distance - total_distance, major_moves_polar_angle);
                            pushLinearMove(new float[] {last_endpoint[0], last_endpoint[1], 0}, new float[] {end_point[0], end_point[1], 0}, move_buffer_without_planning.get(x).target_velocity, move_buffer_without_planning.get(x).current_velocity);
                            last_endpoint = end_point;
                        }
                        if (total_distance < major_move_distance) //We accelerated to target velocity before major move ended so finish move at target velocity
                        {
                            float[] end_point = getPolarLineEndpoint(last_endpoint, major_move_distance - total_distance, major_moves_polar_angle);
